		...
		for ($i=0; $i<count($totes);$i++){
			...
			<td>
				...
				if(file_exists ("../img/".$totes[$i]->getFoto())){
					echo("<a href='DadesActor.php?nif=".$totes[$i]->getNif()."'><img id ='img' src='../img/".$totes[$i]->getFoto()."'></a>");
				}else{
					echo("<a href='DadesActor.php?nif=".$totes[$i]->getNif()."'><img id ='img' src='../img/notfound.jpg'></a>");
				}
